...
def correction_layer(nn_params, q, batch, *, model, E, E_pinv):
    eta = jnp.concatenate([batch["b"], batch["d"]], axis=-1)
    return model.apply({"params": nn_params}, q, batch["lam"], E, E_pinv, eta)


def f_and_grad(x, batch, *, Q):
    Qx = x @ Q
    p = batch["p"]
    f = 0.5 * jnp.sum(x * Qx, axis=-1) + jnp.sum(p * x, axis=-1)
    return f, Qx + p
...
def metrics(x, data, *, Q, A, C):
    f, _ = f_and_grad(jnp.asarray(x), {"p": jnp.asarray(data["p"])}, Q=Q)
    A = np.asarray(A)
    C = np.asarray(C)
...
def main() -> None:
    ...
    val_data = {
        ...
        "p": p_all[n_train : n_train + n_val],
        "b": b_all[n_train : n_train + n_val],
        "d": d_all[n_train : n_train + n_val],
    }

    Q_mat = jnp.array(data["Q"])    # (n_var, n_var)
    ...
    unroll = Unroll(
        cfg,
        model_step=partial(correction_layer, model=model, E=E, E_pinv=EtE_inv),
        f_and_grad=partial(f_and_grad, Q=Q_mat),
        kkt_residual=partial(kkt_residual, A=A, C=C),
        metrics=partial(metrics, Q=Q_mat, A=A, C=C),
    )
    trainer = Train(unroll, optimizer)

    n_test = n_total - n_train - n_val
    print("=" * 80)
    print("Training QP — HUANet (correction mode)")
    print("=" * 80)
