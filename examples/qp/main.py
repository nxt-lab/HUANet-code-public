...
def make_l2o_solver(
    ...
    @jax.pmap
    def solve(lam_batch: jnp.ndarray, b_batch: jnp.ndarray, d_batch: jnp.ndarray) -> jnp.ndarray:
        n_samples = lam_batch.shape[0]
        w_init = jnp.zeros((n_samples, n_ineq))
        v_init = jnp.zeros_like(w_init)
        x_init = jnp.zeros((n_samples, n_var))
        eta = jnp.concatenate([b_batch, d_batch], axis=-1)

        def body_fn(_, carry):
            w_k, v_k, _ = carry
            q = w_k - v_k / rho
            x_next, s_next, _ = model.apply({"params": nn_params}, q, lam_batch, E, E_pinv, eta)
...
def baseline_solver(
    ...
    x_batch, solve_times = [], []
    for b_sample, d_sample, p_sample in zip(b_samples, d_samples, p):
        x = cp.Variable(n_var)
        problem = cp.Problem(
            cp.Minimize(0.5 * cp.quad_form(x, cp.psd_wrap(Q)) + p_sample @ x),
            [A @ x == b_sample, C @ x <= d_sample],
        )
        start = time.perf_counter()
        problem.solve(solver=solver_name, warm_start=True, verbose=False)
        elapsed = time.perf_counter() - start
        x_batch.append(x.value)
        solve_times.append(elapsed)
    return np.array(x_batch), np.array(solve_times, dtype=float)


def primal_solver(q, Q, p, A, C, b, d, rho):
    x = cp.Variable(Q.shape[0])
    s = cp.Variable(C.shape[0])
    problem = cp.Problem(
        cp.Minimize(0.5 * cp.quad_form(x, cp.psd_wrap(Q)) + p @ x + 0.5 * rho * cp.sum_squares(s - q)),
        [A @ x == b, C @ x + s == d],
    )
    start = time.perf_counter()
...
if __name__ == "__main__":
    ...
    if pad:
        lam = jnp.concatenate([lam, jnp.zeros((pad, n_var), dtype=lam.dtype)], axis=0)
        b = jnp.concatenate([b, jnp.zeros((pad, n_eq), dtype=b.dtype)], axis=0)
        d = jnp.concatenate([d, jnp.zeros((pad, n_ineq), dtype=d.dtype)], axis=0)
    # Warm up
    print("Running L2O...")
    lam0 = jnp.zeros((n_devices, 1, n_var), dtype=lam.dtype)
    b0 = jnp.zeros((n_devices, 1, n_eq), dtype=b.dtype)
    d0 = jnp.zeros((n_devices, 1, n_ineq), dtype=d.dtype)
    _ = l2o_solve(lam0, b0, d0).block_until_ready()

    # Per-batch timing
    l2o_times = []
    x_parts = []
    for i in range(0, len(b), n_devices):
        end = i + n_devices
        lam_i = lam[i:end, None, :]
        b_i = b[i:end, None, :]
        d_i = d[i:end, None, :]
        start = time.perf_counter()
        x_i = l2o_solve(lam_i, b_i, d_i)
    ...
    x_clarabel, solve_times_clarabel = baseline_solver(b_test_np, d_test_np, Q_np, p_test_np, A_np, C_np, n_var, cp.CLARABEL)
    # ---- OSQP ----
    print("Running OSQP...")
    x_osqp, solve_times_osqp = baseline_solver(b_test_np, d_test_np, Q_np, p_test_np, A_np, C_np, n_var, cp.OSQP)

    # ---- ADMM ----
    x_admm = np.zeros((n_test, n_var))
    times_admm = np.zeros(n_test)
    print(f"Running ADMM on {n_test} samples...")
    for i, (p_i, b_i, d_i) in enumerate(zip(p_test_np, b_test_np, d_test_np)):
        x_admm[i], times_admm[i] = admm(
            lambda q: primal_solver(q, Q_np, p_i, A_np, C_np, b_i, d_i, rho),
            n_ineq=n_ineq,
            max_iter=max_iter,
            tol=tol,
            rho=rho,
    ...
    report_benchmark(
        summary_path,
        samples={"lam": lam_test_np, "p": p_test_np, "b": b_test_np, "d": d_test_np},
        matrices={"Q": Q_np, "A": A_np, "C": C_np},
        solvers={
            "clarabel": (x_clarabel, solve_times_clarabel),
            "our_method": (x_pred, l2o_times),
            "osqp": (x_osqp, solve_times_osqp),
        },
