...
def benchmark_metrics(
    ...
    obj_pred = 0.5 * np.sum(y_pred * (y_pred @ Q), axis=1) + np.sum(y_pred * p, axis=1)
    obj_true = 0.5 * np.sum(y_true * (y_true @ Q), axis=1) + np.sum(y_true * p, axis=1)
...
def compute_metrics(y, times, y_ref, samples, matrices):
    Q, A, C = matrices["Q"], matrices["A"], matrices["C"]
    p, b, d = samples["p"], samples["b"], samples["d"]
    obj = 0.5 * np.sum(y * (y @ Q), axis=1) + np.sum(y * p, axis=1)
    return {
        "y": y,
        "times": times,
        "obj": obj,
        ...
        "metrics": benchmark_metrics(y, y_ref, Q, p, A, C, b, d, times),
    }
...
def report_benchmark(summary_path, samples, matrices, solvers, sequential_time) -> None:
    all_solvers = [
        ...
        ("osqp", *solvers["osqp"], "OSQP Metrics (vs Clarabel)"),
    ]

    save_dict = {
        **samples,
        "n_var": matrices["Q"].shape[0],
